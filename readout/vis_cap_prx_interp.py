...
filename_in = 'hum_test19'
fileext_in = '.txt'
filename_out = 'hum_test19'
...
for i, y_CAPi in enumerate(y_CAP):
    y_CAP_coeffs.append(splrep(x, y_CAPi, s=smoothing))
for i, y_PRXi in enumerate(y_PRX):
    y_PRX_coeffs.append(splrep(x, y_PRXi, s=smoothing))
...
# Populate capacitance/light derivate value graphs
for i_ax3, ax3i in enumerate(ax3):
    if i_ax3 < len(Constants.COL_CAP):
        ax3i.plot(x, dy_CAP[i_ax3], '#ff000060', linestyle='solid')
        ax3i.set_ylim(Constants.DCAP_YLIM[i_ax3])
    else:
        ax3i.plot(x, dy_PRX[i_ax3 - len(Constants.COL_CAP)], '#ff000060', linestyle='solid')
